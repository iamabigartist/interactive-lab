...
import {acos, chain, compile, derivative, evaluate, exp, parse, parser, string} from "mathjs";
...
export function TaylorExpansion(f: string, variable: string, x0: number, order: number) {
    ...
    let expansion = "";
    for (let i = 0; i <= order; i++) {
        expansion += `+((${variable}-${x0})^${order})*${derivatives[i]}/${order}!`;
    }
    return expansion;
}
...
const DrawFunction: FC = () => {
    ...
    let ori_f = "sin(x)+1/2*sin(2x)"
    let order = 5;
    let range: [number, number] = [-5, 5];
    let step = 0.01;

    let cur_curve = compile(TaylorExpansion(ori_f, "x", 0, order));
